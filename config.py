...
# Telegram отклоняет счёт (CURRENCY_TOTAL_AMOUNT_INVALID), если сумма меньше
# минимума по валюте — а минимум примерно равен $1 (для RUB это ~80-100 ₽ по
# текущему курсу). У наших заказов минимальная сумма может быть в разы меньше
# (например 1.8 ₽ за 10 подписчиков), поэтому карту показываем только когда
# сумма заказа выше этого порога — с запасом на случай колебаний курса.
MIN_CARD_AMOUNT_RUB = 100
